...
Check if changes require updates to `docs/` or `README.md`:
- New features, commands, or flags
...
- Run `golangci-lint run ./...` to catch lint errors
- Run `go test -race -count=1 ./...` to verify unit tests pass
- Run `govulncheck ./...` to check for known vulnerabilities
- Address any failures before proceeding

If you have [mise](https://mise.jdx.dev) installed, you can run:
```bash
mise run fmt
mise run lint
mise run test
...
2. **Commit changes** following the commit message guidelines:
   ...
   ```bash
   git add <files>
   git commit --signoff -m "feat(runner): add runner list command

   Adds the `semctl runner list` command with table and JSON output.
   Includes unit tests and golden file fixtures."
   ```
...
<context>
When working on GitHub-issue-related changes, include the issue number and relevant information in `change.md` and commit messages.

Example commit message referencing an issue:
```
...
<context>
All commits must include a `Signed-off-by` line (use `git commit --signoff`).

The DCO requirement is enforced by:
...
<example>
```
User: "Add a runner list command"
...
AI: [Runs go fmt, golangci-lint, go test, govulncheck]
AI: [Creates branch, commits with DCO sign-off, pushes to origin]
AI: "Changes have been pushed to branch 'feature/runner-list'.
     You can now create a PR on GitHub."
```
</example>

---
<- Back to [README.md](../README.md) | [Contributing Guide](../CONTRIBUTING.md)
