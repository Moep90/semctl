...
func TestListCommand(t *testing.T) {
	...
	root.SetArgs([]string{"inventory", "list", "--host", srv.URL, "--project", "infra"})
	err := root.Execute()
	...
	data, _ := io.ReadAll(r)
	out := string(data)
	if !strings.Contains(out, "prod-hosts") {
		t.Fatalf("expected prod-hosts in output, got: %s", out)
	}
	if !strings.Contains(out, "dev-hosts") {
		t.Fatalf("expected dev-hosts in output, got: %s", out)
	}
}
...
func TestGetCommand(t *testing.T) {
	...
	root := newTestRoot(nil)
	root.SetArgs([]string{"inventory", "get", "dev-hosts", "--host", srv.URL, "--project", "infra"})
	err := root.Execute()
