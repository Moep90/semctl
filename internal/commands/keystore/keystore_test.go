...
func TestListCommand(t *testing.T) {
	...
	root.SetArgs([]string{"keystore", "list", "--host", srv.URL, "--project", "infra"})
	err := root.Execute()
	...
	if !strings.Contains(out, "deploy-key") {
		t.Fatalf("expected deploy-key in output, got: %s", out)
	}
	if !strings.Contains(out, "vault-pass") {
		t.Fatalf("expected vault-pass in output, got: %s", out)
	}
}
...
func TestGetCommand(t *testing.T) {
	...
	root := newTestRoot(nil)
	root.SetArgs([]string{"keystore", "get", "vault-pass", "--host", srv.URL, "--project", "infra"})
	err := root.Execute()
...
func newTestRoot(out *bytes.Buffer) *cobra.Command {
	...
	root.AddCommand(NewKeystoreCommand())
	if out != nil {
		root.SetOut(out)
		root.SetErr(out)
	}
