...
// Command gen-commands-doc writes docs/commands.md from the wired command tree.
// Invoked via `go generate ./internal/commands/...`.
package main
...
func main() {
	root, err := moduleRoot()
	if err != nil {
		fmt.Fprintln(os.Stderr, "gen-commands-doc:", err)
		os.Exit(1)
	}
	doc := commands.RenderMarkdownDocs(commands.NewRootCommand("dev"))
